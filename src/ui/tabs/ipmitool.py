"""IPMI tool tab implementation."""

from nicegui import ui

from src.mixins.multi_screen import MultiScreenMixin
from src.models.configurations import AppConfig
from src.ui.components.selector import Selector
from src.ui.tabs.base import BasePanel, BaseTab
from src.utils.connect import Ssh

NAME = "ipmitool"
LABEL = "IPMI Tool"


class IpmitoolTab(BaseTab):
    ICON_NAME: str = "settings_remote"

    def __init__(self, build: bool = False) -> None:
        super().__init__(NAME, LABEL, self.ICON_NAME)
        if build:
            self.build()

    def build(self) -> None:
        super().build()


class IpmitoolPanel(BasePanel, MultiScreenMixin):
    def __init__(
        self,
        build: bool = False,
        app_config: AppConfig = None,
        ssh: Ssh = None,
        host_handler=None,
        icon: ui.icon = None,
    ):
        BasePanel.__init__(self, NAME, LABEL, IpmitoolTab.ICON_NAME)
        MultiScreenMixin.__init__(self)
        self._app_config = app_config
        self._ssh = ssh
        self._host_handler = host_handler
        self._icon = icon
        if build:
            self.build()

    def build(self):
        with ui.tab_panel(self.name).classes("w-full h-screen"):
            self._build_controls_base("IPMI Tool")
            self._build_content_base()

    def _build_screen(self, screen_num, classes):
        with ui.card().classes(classes), ui.expansion(f"Host {screen_num}", icon="computer").classes("w-full"):
            if self._host_handler:
                Selector(
                    self._host_handler._connect_route,  # noqa: SLF001
                    self._host_handler._routes,  # noqa: SLF001
                    lambda conn_id, s=screen_num: self._on_connection_change(conn_id, s),
                ).build()
            ui.button("IPMI Command", on_click=lambda s=screen_num: self._run_ipmi_command(s)).classes(
                "bg-purple-300 hover:bg-purple-400 text-purple-900 mt-2"
            )
            ui.label(f"IPMI tools for host {screen_num}").classes("mt-4")

    def _run_ipmi_command(self, screen_num):
        ui.notify(f"Running IPMI command for screen {screen_num}", color="info")
