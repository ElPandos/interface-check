...
class NetworkStatCollector(StatCollector):
    ...
    def collect(self) -> StatPoint:
        if not self._connection:
            return StatPoint(dt.now(tz=UTC), 0.0)
    ...
    def collect(self) -> StatPoint:
        ...
        if result.success:
            try:
                ...
                return StatPoint(
                    timestamp=dt.now(tz=UTC), value=cpu_usage, metadata={"unit": "percent"}
                )
            except ValueError:
                pass
    ...
    def collect(self) -> StatPoint:
        ...
        if result.success:
            try:
                ...
                return StatPoint(
                    timestamp=dt.now(tz=UTC), value=mem_usage, metadata={"unit": "percent"}
                )
            except ValueError:
                pass
...
class Statistics:
        ...
        for name, collector in self._collectors.items():
            try:
                ...
                results[name] = StatPoint(
                    timestamp=dt.now(tz=UTC), value=0.0, metadata={"error": True}
                )
    ...
    def export_data(self, collector_name: str, hours: int = 24) -> dict[str, Any]:
        ...
        return {
            ...
            "export_timestamp": dt.now(tz=UTC).isoformat(),
        }
