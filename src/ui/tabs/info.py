...
class InfoPanel(BasePanel):
    def __init__(
        ...
        super().__init__(NAME, LABEL, InfoTab.ICON_NAME)
    ...
    def _build_screen(self, screen_num, classes):
        with ui.card().classes(classes), ui.expansion(f"Host {screen_num}", icon="computer").classes("w-full"):
            ...
            ui.button("Get Info", on_click=lambda s=screen_num: self._get_info(s)).classes(
    ...
    def _get_info(self, screen_num):
        ui.notify(f"Getting info for host {screen_num}", color="info")
