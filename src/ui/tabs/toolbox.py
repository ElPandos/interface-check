...
class ToolboxPanel(BasePanel):
    def __init__(
        ...
        super().__init__(NAME, LABEL, ToolboxTab.ICON_NAME)
    ...
    def build(self):
        with ui.tab_panel(self.name).classes("w-full h-screen"), ui.column().classes("w-full h-full p-4"):
            with ui.card().classes("w-full h-full p-6 shadow-lg bg-white border border-gray-200"):
                with ui.row().classes("w-full items-center gap-3 mb-6"):
                    ui.icon("home_repair_service", size="lg").classes("text-blue-600")
    ...
    def _get_tool_icon(self, tool: SoftwareTool) -> str:
        """Get appropriate icon for each tool."""
        icons = {
            SoftwareTool.MLXCONFIG: "settings",
            SoftwareTool.MLXLINK: "link",
            SoftwareTool.IPMITOOL: "computer",
            SoftwareTool.ETHTOOL: "network_check",
        }
        return icons.get(tool, "build")

    def _select_tool(self, tool: SoftwareTool):
